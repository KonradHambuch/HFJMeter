...
function setup_org_peer_context() {
    ...
    export CORE_PEER_TLS_ROOTCERT_FILE=${CRYPTO}/peerOrganizations/${2}.example.com/users/Admin@${2}.example.com/tls/ca.crt
    export CORE_PEER_TLS_CERT_FILE=${CRYPTO}/peerOrganizations/${2}.example.com/users/Admin@${2}.example.com/tls/client.crt
    export CORE_PEER_TLS_KEY_FILE=${CRYPTO}/peerOrganizations/${2}.example.com/users/Admin@${2}.example.com/tls/client.key
}
...
function update_anchor() {
    ...
    if peer channel update --orderer ${ORDERER_URL} --tls --cafile ${ORDERER_CAFILE} --channelID ${CHANNEL_NAME} --file ${CONFIG_ROOT}/tls/${CHANNEL_NAME}-$1-anchor.tx
    then
...
function instantiate_chaincode() {
    ...
    if peer chaincode instantiate --orderer ${ORDERER_URL} --tls --cafile ${ORDERER_CAFILE} --channelID ${CHANNEL_NAME} --name $1 --version $2 --policy "AND('CentralBankOrgMSP.member','FIOrgMSP.member')" --ctor '{"Args":[]}'
    then
...
function ping_chaincode() {
    ...
    if peer chaincode query --channelID ${CHANNEL_NAME} --name $1 --ctor '{"Args":["ping"]}' --tls --peerAddresses "peer0.${2}:7051" --tlsRootCertFiles "${CRYPTO}/peerOrganizations/$2/peers/peer0.$2/msp/tlscacerts/tlsca.$2-cert.pem"
    then
...
if peer channel create --orderer ${ORDERER_URL} --tls --cafile ${ORDERER_CAFILE} --channelID ${CHANNEL_NAME} --file ${CONFIG_ROOT}/tls/${CHANNEL_NAME}.tx
then
    log "Channel created"
else
